...
class STResNet(nn.Module):
    def __init__(self, in_features, out_features, num_blocks):
        ...
        self.fc1 = nn.Linear(in_features, out_features)
        self.leaky_relu = nn.LeakyReLU(inplace=True)
        self.res_blocks = nn.ModuleList([STResNetBlock(out_features, out_features) for _ in range(num_blocks)])
        self.fc2 = nn.Linear(out_features, out_features)
...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, grid_pe=0, skips=[], args=None):
        ...
        if self.args.static_mlp:
            self.static_mlp = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.W, self.W), nn.LeakyReLU(), nn.Linear(self.W, 1))
    ...
    def create_net(self):
        ...
        if self.no_grid:
            self.feature_out = STResNet(4, self.W, self.D)
        else:
            self.feature_out = STResNet(mlp_out_dim + grid_out_dim, self.W, self.D)
        
        self.pos_deform = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.W, self.W), nn.LeakyReLU(), nn.Linear(self.W, 3))
        self.scales_deform = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.W, self.W), nn.LeakyReLU(), nn.Linear(self.W, 3))
        self.rotations_deform = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.W, self.W), nn.LeakyReLU(), nn.Linear(self.W, 4))
        self.opacity_deform = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.W, self.W), nn.LeakyReLU(), nn.Linear(self.W, 1))
        self.shs_deform = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.W, self.W), nn.LeakyReLU(), nn.Linear(self.W, 16 * 3))
...
class deform_network(nn.Module):
    def __init__(self, args):
        ...
        self.timenet = nn.Sequential(
            nn.Linear(times_ch, timenet_width), nn.LeakyReLU(),
